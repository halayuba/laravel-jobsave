<template>
  <div class="w-full flex justify-around sm:justify-end items-center sm:px-2 mt-4 sm:mt-0">
    ...
    <a
      href="#"
      title="Update as unsuccessful"
      @click.prevent="notSuccessful"
      v-if="submission.status != 'Unsuccessful'"
    >
      <icon
        name="thumb-down"
        class="w-5 h-5 mt-1 fill-current text-gray-700"
      />
    </a>

    <picture v-if="submission.status == 'Unsuccessful'">
      <icon
        name="thumb-down-filled"
        class="w-5 h-5 mt-1 fill-current text-red-800"
      />
    ...
    >
      <icon
        name="handshake"
        class="w-8 h-8 fill-current text-indigo-600"
      />
    </picture>
...
export default {
  ...
  computed: {
    ...
    hasUpcomingInterview () {
      return this.submission.interviews.some(interview => interview.status === "Upcoming")
    },
