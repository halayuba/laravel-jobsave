  ...
  <div class="card w-full sm:w-64">
    ...
    <div class="mt-2 flex justify-between items-center">
      ...
      <div class="flex items-center">
        <h3
          :class="cardHeader"
        >{{ interview.submission.company }}</h3>
      </div>

    </div> <!-- COMPANY -->

    <!-- POSITION -->
    <a class="block mt-6 px-6 text-gray-600 text-xs"
    ...
    <div class="flex flex-col mt-4 px-6">
      ...
      <div class="flex items-baseline">
        <icon name="users" class="w-4 h-4 fill-current text-gray-600" />
        <span class="text-gray-700 text-sm tracking-tight ml-2">{{ interview.interviewer }}</span>
      </div>

      <!-- INTERVIEW DATE / TIME -->
      <div class="mt-4 flex items-center">
        <icon name="calendar" class="w-6 h-6 fill-current text-gray-600 flex-shrink-0" />
      ...
      >
        <icon name="note" class="w-6 h-6 fill-current text-gray-600 flex-shrink-0"
      ...
      <!-- STATUS -->
      <div class="mt-4 flex items-center">
        <icon name="sun" class="w-8 h-8 fill-current text-gray-600" />
    ...
    >
      <div class="pt-4 flex justify-end items-center border-t border-gray-200">
        ...
        >
          <icon name="checkbox" class="w-5 h-auto fill-current text-green-500"/>
        </picture>
...
import dayjs from 'dayjs'
  ...
  export default {
    ...
    computed: {
      cardHeader() {
        if( this.interview.status === 'Upcoming'){
          return 'card_header_current'
        } else if ( this.interview.status === 'Rescheduled' && dayjs().isBefore(dayjs(this.interview.date)) ) {
          return 'card_header_current'
        } else {
          return 'card_header'
        }
